 ...
 *   ✅ app/**               现有路由：落地页（/）、内置演示（/demo/**）、404 页
 *
 * 词典**不**负责 **业务记录内容**——那是数据，不是可翻译的文案：
 *   ⛔ lib/research/**      研究记录内容（问题、论断、原文片段、来源标题）
 *   ⛔ lib/mock-data.ts     Factory 内置演示的账户、活动流、通知
 *
 * 因此 `app/**` 与 `components/**` 里不应再出现任何用户可见的字面文案；
 * 新增页面直接从词典取文案即可。
 *
 * > 产品界面开始出现后，把它的路由加进上面的 ✅ 列表，并同步更新
 * > `tests/support/localization.ts` 的 `LOCALIZED_ROUTES`。
 */
