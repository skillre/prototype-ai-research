...
export interface Finding {
  ...
  /**
   * **写进交付物的措辞。** 它是人写的话，不是投影的输出。
   *
   * ⚠ 不要用 `projectKnownLimitations(data).map(x => x.reason)` 填这里。
   * 那两样东西住在不同的层：
   *
   * ```
   * KnownLimitation              带 id / 日期的**现状记录**（工作区里）
   * Finding.knownLimitations    研究者愿意**署名**的措辞（交付物里）
   * ```
   *
   * 前者的 `reason` 是「我为什么接受它」，后者是「交付物里怎么写它」——
   * 一句话通常不相同。把它们接成一根管道，等于让工作区的随手记录
   * 自动变成对外署名的话。
   */
  knownLimitations: string[]
  /**
   * 上面的措辞**对哪些已接受的局限负责**（Phase G+H 新增）。
   *
   * ## 为什么必须是显式的字段
   *
   * 「这条边界写进交付物了吗」这个问题，如果靠比对字符串来回答，就永远
   * 只能是一句猜测。有了 refs：
   *
   * ```
   * refs 里有、且对应的局限仍然存在   → 已写入，且现状一致
   * refs 里有、但事实后来变了         → **交付物在夸大一条已经不存在的限制**
   * 已接受、但 refs 里没有            → 还没写进交付物（有候选措辞，等人决定）
   * ```
   *
   * 中间那一条是这一整个产品最不能静默的一种坏状态：交付物里写着一条
   * 边界，而工作区里那条边界的洞已经被填上了。它只能被查出来，
   * 不能被猜出来。
   *
   * 允许为空：手写的边界（不来自任何处置）不需要 ref——`knownLimitations`
   * 与 `knownLimitationRefs` **不要求**一一对应，因为交付物里的措辞
   * 可以合并几条局限，也可以完全不引用任何一条。
   */
  knownLimitationRefs: (Id | null)[]
  /**
   * 这份交付物是哪一刻生成的。
   *
   * 交付物必须带日期：它是一份**快照**，而工作区是活的。两者会分叉，
   * 分叉本身不是缺陷——但一份不写明自己是什么时候定稿的交付物，
   * 会让读者以为它反映的是当下。
   */
