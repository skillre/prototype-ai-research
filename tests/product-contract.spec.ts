...
test.describe("no generated semantics", () => {
  ...
  test("the shipped contract declares this product's own invariants", () => {
    // Inverse of the baseline's rule, on purpose. In `prototype-starter` a
    // domain invariant would be inherited residue; here it is the whole point:
    // this repository IS a product, and `product-contract.json` is where its 18
    // semantic invariants live. What must *not* survive is the other direction —
    // a baseline-level `factory.*` invariant inherited without the core spec
    // that holds it (this product does not ship `tests/core-neutrality.spec.ts`).
    ...
    // The gate's own invariant: registered by this very spec file, so the
    // contract has to declare it or the bidirectional check reports
    // `contract/undeclared`.
...
/* -------------------------------------------------------------------------- */
/* §7.9 — this product's own invariants, read back                             */
/* -------------------------------------------------------------------------- */
 ...
 * expressive enough for a real product's invariants, which at that time had no
 * home outside `tests/invariants.spec.ts` (F11).
 *
 * This repository *is* that product, and it has since adopted the contract, so
 * the same probe now runs against its own spec — relatively, and with one extra
 * claim the read-only version could not make: every id-shaped title is backed by
 * an actual `invariant("<id>", …)` registration.
 */
test.describe("本产品的不变量：标题与机器可读 id 并存", () => {
  const archived = join(ROOT, "tests/invariants.spec.ts")
  ...
  test("its describe titles are already nearly ids — this contract gives them a home", () => {
    ...
    try {
      ...
      test.skip(true, "本仓没有 tests/invariants.spec.ts")
      return
    }
    ...
    const expressive = {
      ...
      invariants: idShaped.map((id) => ({
        id,
        statement: "从本产品的 invariant 标题恢复（只读验证，未修改该文件）。",
        enforcement: "test",
      })),
    }
    expect(errors(expressive)).toEqual([])

    // The claim the read-only version could not make: the titles a human reads
    // and the ids the gate matches are now **the same set** — every id-shaped
    // title has a registration right next to it.
    //
    // The needle is assembled from fragments on purpose: spelling it out would
    // put a literal `inv("…")`-shaped string in this file, and the registration
    // scanner (which strips comments, not string contents) would then read this
    // assertion as a registration of its own id.
    const call = ["inv", "ariant("].join("")
    for (const id of idShaped) {
      expect(
        source.includes(`${call}"${id}"`),
        `标题 ${id} 有 id 形状，但文件里没有对应的 ${call}"${id}") 登记`,
      ).toBe(true)
    }
  })
})
